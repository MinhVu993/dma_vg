...
class vgDormCtrl extends Controller 
{
    ...
    private function getLevelInfo($empno, $dataAppFlow, $targetLevel = null) {
        ...
        // Scan all levels to find all possible permissions
...
// Helper function to calculate next valid index
...
private function formatOccupantsData($nameArray, $reason = '')
{
    ...
    try {
        ...
        return collect($nameArray)->map(function($person) use ($reason) {
            ...
            // Extract location value correctly
            $location = '';
...
private function generateVisaXml($occupants, $filename = null) 
{
    try {
        ...
        foreach ($occupants as $index => $occupant) {
            ...
            $gender = '';
            if (strpos($occupant['gender'] ?? '', 'Nam') !== false || 
            strpos($occupant['gender'] ?? '', '男') !== false || 
            strpos($occupant['gender'] ?? '', 'M') !== false) {
                $gender = 'M';
            } else if (strpos($occupant['gender'] ?? '', 'Nữ') !== false || 
...
private function buildSubmitterMessage($submitter, $item) {
    try {
        ...
        // $excelPath = $this->generateVisaExcel($occupants, 'Visa.xlsx');
        $excelPath = $this->generateVisaExcel($occupants, 'visa_info_' . $item->id . '.xlsx');
        $xmlPath = $this->generateVisaXml($occupants, 'visa_info_' . $item->id . '.xml');
        ...
        // Add both attachments
        if ($excelPath && file_exists($excelPath)) {
            $visaMailData['attachments'] = $excelPath;
        }
        if ($xmlPath && file_exists($xmlPath)) {
            // If attachments already exists, append XML path
            if (isset($visaMailData['attachments'])) {
                $visaMailData['attachments'] .= '|' . $xmlPath;
            } else {
                $visaMailData['attachments'] = $xmlPath;
            }
        }
        ...
        public static function getFilteredRequests($userData, $includeDeputy = true) 
        {
            ...
            foreach ($allRequests as $request) {
                try {
                    ...
                    (is_string($request->status) ? json_decode($request->status, true) : null);
                    
                    $approval = is_array($request->approval) ? $request->approval :
                    (is_string($request->approval) ? json_decode($request->approval, true) : null);
                    ...
                    else if (is_array($status) && isset($status[0]['dept'], $status[1]['ga']) &&
                    $status[0]['dept'] === "true" && $status[0]['stt'] === "accept" &&
                    ($status[1]['ga'] === "false" || $status[1]['stt'] === "waiting ga")) {
                        $targetLevel = 1;
                    }
                    else if (is_array($status) && isset($status[1]['ga'], $status[2]['smp']) &&
                    $status[1]['ga'] === "true" && $status[1]['stt'] === "accept" &&
                    ($status[2]['smp'] === "false" || $status[2]['stt'] === "waiting smp")) {
                        $targetLevel = 2;
                    }
                    else if (is_array($status) && isset($status[2]['smp'], $status[3]['gm']) &&
