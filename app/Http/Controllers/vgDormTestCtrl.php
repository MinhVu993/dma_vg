...
class vgDormCtrl extends Controller
{
    ...
    private function getLevelInfo($empno, $dataAppFlow, $targetLevel = null)
    {
        ...
        // Support array of empnos (e.g. empno + group alias)
        $empnoList = is_array($empno)
            ? array_map('strtoupper', array_filter($empno))
            : [strtoupper($empno)];
        ...
        foreach ($dataAppFlow as $index => $level) {
            if (isset($level['managers']) && is_array($level['managers'])) {
                ...
                foreach ($level['managers'] as $manager) {
                    if (in_array(strtoupper($manager['empno']), $empnoList)) {
                        if (!isset($foundLevels[$index]) || $foundLevels[$index]['type'] !== 'direct') {
                            $foundLevels[$index] = [
                                'level' => sprintf(
                                    '%s-%s-%s',
                                    $manager['empno'],
                ...
                // Check deputy role (only if no direct match found for this index)
                foreach ($level['managers'] as $manager) {
                    if (isset($manager['deputies']) && is_array($manager['deputies'])) {
                        foreach ($manager['deputies'] as $deputy) {
                            if (in_array(strtoupper($deputy['empno']), $empnoList)) {
                                if (!isset($foundLevels[$index])) {
                                    $foundLevels[$index] = [
                                        'level' => sprintf(
                                            '%s-%s-%s',
                                            $deputy['empno'],
        ...
        if ($targetLevel !== null && isset($foundLevels[$targetLevel])) {
            ...
            $nextValidIndex = $this->calculateNextValidIndex($dataAppFlow, $targetLevel);
        ...
        if (!empty($foundLevels)) {
            ...
            $nextValidIndex = $this->calculateNextValidIndex($dataAppFlow, $lowestLevel);
            $levelInfo['nextValidIndex'] = $nextValidIndex;
            return $levelInfo;
        }
    ...
    public function acceptRequest(Request $req)
    {
        try {
            ...
            // Build empno list: include both empno and group (alias empno used in dataAppFlow)
            $empnoList = array_values(array_unique(array_filter([
                $currentUser['empno'] ?? null,
                $currentUser['group'] ?? null,
            ])));
            $levelInfo = $this->getLevelInfo($empnoList, $dataAppFlow, $targetLevel);
            ...
            $emailData = [
                ...
                'location' => !empty($occupants[0]['location']) ? $occupants[0]['location'] : '',
                'gender' => !empty($occupants[0]['gender']) ? $occupants[0]['gender'] : '',
                'reason' => $item->reason
            ];
            ...
            if (!$isLastLevel && isset($dataAppFlow[$nextIndex]) && isset($dataAppFlow[$nextIndex]['managers'])) {
                ...
                try {
                    ...
                    \Log::warning('Notification timeout, continuing: ' . $e->getMessage());
                }
            } else {
    ...
    public function acceptRequest_test(Request $req)
    {
        try {
            $status = $req->status;
            $approval = $req->approval;
    ...
    private function formatOccupantsData($nameArray, $reason = '')
    {
        ...
        try {
            ...
            return collect($nameArray)->map(function ($person) use ($reason) {
                if (!is_array($person)) {
                ...
                if (isset($person['location'])) {
                    if (is_array($person['location']) && isset($person['location']['loc'])) {
                        $location = $person['location']['loc'];
                    } else if (is_string($person['location'])) {
                        $location = $person['location'];
                    }
                }

                $gender = isset($person['gender']) ?
                    (strtoupper($person['gender']) === 'M' ? 'Nam/男' : 'Nữ/女') : '';

                $id_number = '';
                if (!empty($person['doc_number'])) {
                    $id_number = $person['doc_number'];
                } else if (!empty($person['citizen_id'])) {
                    $id_number = $person['citizen_id'];
                } else if (!empty($person['passport_number'])) {
                ...
                return [
                    ...
                    'province_id' => (string)($person['province_id'] ?? ''),
                    'ward_id' => (string)($person['ward_id'] ?? ''),
                    'detailed_address' => (string)($person['detailed_address'] ?? '')
                ];
            })->all();
        } catch (\Exception $e) {
    ...
    private function generateVisaXml($occupants, $filename = null)
    {
        try {
            $foreignOccupants = array_filter($occupants, function ($occupant) {
                ...
                $isVn = (strpos($nation, 'viet') !== false || $nation === 'vnm' ||
                         strpos($rawNation, 'viet') !== false || $rawNation === 'vnm');
                return !$isVn;
            });
            ...
            $xml = new \SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><KHAI_BAO_TAM_TRU></KHAI_BAO_TAM_TRU>');
            foreach ($foreignOccupants as $index => $occupant) {
                $khach = $xml->addChild('THONG_TIN_KHACH');
                $khach->addChild('so_thu_tu', $index + 1);
                $khach->addChild('ho_ten', htmlspecialchars($occupant['name'] ?? ''));
                $khach->addChild('ngay_sinh', isset($occupant['birth_date']) ? date('d/m/Y', strtotime($occupant['birth_date'])) : '');
                ...
                $khach->addChild('ngay_den', isset($occupant['start_date']) ? date('d/m/Y', strtotime($occupant['start_date'])) : '');
                $khach->addChild('ngay_di_du_kien', isset($occupant['end_date']) ? date('d/m/Y', strtotime($occupant['end_date'])) : '');
                $khach->addChild('ngay_tra_phong', isset($occupant['end_date']) ? date('d/m/Y', strtotime($occupant['end_date'])) : '');
            }
    ...
    public function generateVisaExcel($occupants, $filename = null)
    {
        try {
            ...
            // Set headers
            $headers = ['STT', 'Họ và tên', 'Ngày sinh', 'QT', 'Số HC', 'Ngày đến', 'Ngày đi DK', 'Ngày trả phòng'];
            ...
            foreach ($occupants as $index => $occupant) {
                $data = [
                    $index + 1,
                    $occupant['name'] ?? '',
                    isset($occupant['birth_date']) ? date('d/m/Y', strtotime($occupant['birth_date'])) : '',
                    $occupant['nation'] ?? '',
                    $occupant['id_number'] ?? '',
                    isset($occupant['start_date']) ? date('d/m/Y', strtotime($occupant['start_date'])) : '',
                    isset($occupant['end_date']) ? date('d/m/Y', strtotime($occupant['end_date'])) : '',
                    isset($occupant['end_date']) ? date('d/m/Y', strtotime($occupant['end_date'])) : ''
                ];
    ...
    private function generateAccommodationExcel($occupants, $filename = null)
    {
        try {
            ...
            $sheet->setCellValue('A1', "SỔ TIẾP NHẬN LƯU TRÚ住宿登記簿\nXÃ TRUÔNG MÍT中密社\nCÔNG TY TNHH CAN SPORTS VN越南志寧責任有限公司");
            ...
            $headerColumns = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'M', 'N', 'O', 'P'];
            foreach ($headerColumns as $index => $col) {
                if ($col !== 'L') { // Skip L as it's merged with K
                    $headerIndex = $index >= 10 ? $index + 1 : $index;
                    if ($headerIndex < count($headers)) {
                        $sheet->setCellValue($col . '7', $headers[$headerIndex]);
                    }
                }
            }
            ...
            foreach ($occupants as $index => $occupant) {
                ...
                if (
                    strpos($occupant['gender'] ?? '', 'Nam') !== false ||
                    strpos($occupant['gender'] ?? '', '男') !== false ||
                    strtoupper($occupant['gender'] ?? '') === 'M'
                ) {
                    $gender = 'Male';
                } else if (
                    strpos($occupant['gender'] ?? '', 'Nữ') !== false ||
                    strpos($occupant['gender'] ?? '', '女') !== false ||
                    strtoupper($occupant['gender'] ?? '') === 'F'
                ) {
                    $gender = 'Female';
                }
                ...
                $data = [
                    ...
                    isset($occupant['birth_date']) ? date('d/m/Y', strtotime($occupant['birth_date'])) : '', // Ngày sinh
                    $gender, // Giới tính
                    ...
                    isset($occupant['start_date']) ? date('d/m/Y', strtotime($occupant['start_date'])) : '', // Bắt đầu
                    isset($occupant['end_date']) ? date('d/m/Y', strtotime($occupant['end_date'])) : '', // Kết thúc
                    '', // Địa chỉ lưu trú
            ...
            // Save file
            $filename = $filename ?? 'so_tiep_nhan_luu_tru_' . date('YmdHis') . '.xlsx';
            ...
            \Log::error('Accommodation Excel generation error: ' . $e->getMessage());
            return null;
        }
    }
    private function generateVnImportExcel($occupants, $submitter)
    {
        try {
            ...
            $vnOccupants = array_filter($occupants, function ($occupant) {
                $nation = strtolower($occupant['nation'] ?? '');
                $rawNation = strtolower($occupant['raw_nation'] ?? '');
                return strpos($nation, 'viet') !== false || $nation === 'vnm' ||
                       strpos($rawNation, 'viet') !== false || $rawNation === 'vnm';
            });

            file_put_contents(base_path('error_log.txt'), "DEBUG: Occupants count: " . count($occupants) . ", VN Occupants count: " . count($vnOccupants) . "\n", FILE_APPEND);
            ...
            if (!file_exists($templatePath)) {
                file_put_contents(base_path('error_log.txt'), "VN Template not found: {$templatePath}\n", FILE_APPEND);
            ...
            foreach ($vnOccupants as $person) {
                $gender = (strpos($person['gender'] ?? '', 'Nam') !== false || strtoupper($person['gender'] ?? '') === 'M') ? 'M - Nam' : 'F - Nữ';
                ...
                if (!empty($person['doc_type'])) {
                    ...
                    if ($docTypeItem) {
                        ...
                        if (strpos($name, 'CCCD') !== false || strpos($name, 'Căn cước') !== false) {
                            $docTypeValue = '1 - Thẻ CCCD';
                        } else if (strpos($name, 'CMND') !== false || strpos($name, 'Chứng minh') !== false) {
                            $docTypeValue = '2 - Thẻ CMND';
                        } else if (strpos($name, 'lái xe') !== false) {
                            $docTypeValue = '3 - Giấy phép lái xe';
                        } else if (strpos($name, 'chiếu') !== false || strpos($name, 'Passport') !== false) {
                ...
                if (!empty($person['res_type'])) {
                    ...
                    if ($resTypeItem) {
                        if (strpos($resTypeItem->name, 'Thường trú') !== false) {
                            $resTypeValue = '1 - Thường trú';
                        } else if (strpos($resTypeItem->name, 'Tạm trú') !== false) {
                ...
                if (!empty($person['res_reason'])) {
                    ...
                    if ($resReasonItem) {
                        ...
                        } else if (strpos($name, 'Công tác') !== false) {
                            $resReasonValue = '2 - Công tác';
                        } else if (strpos($name, 'Học tập') !== false) {
                            $resReasonValue = '3 - Học tập';
                        } else if (strpos($name, 'Lao động') !== false) {
                ...
                if (!empty($person['province_id'])) {
                    $prov = provinceMdl::where('code', $person['province_id'])->first();
                    if ($prov) {
                ...
                if (!empty($person['ward_id'])) {
                    $w = dormWardMdl::where('code', $person['ward_id'])->first();
                    if ($w) {
                        $wardValue = "{$w->code} - {$w->name}";
                    }
                }

                $birthDate = !empty($person['birth_date']) ? date('d/m/Y', strtotime($person['birth_date'])) : '';
                $startDate = !empty($person['start_date']) ? date('d/m/Y', strtotime($person['start_date'])) : '';
                $endDate = !empty($person['end_date']) ? date('d/m/Y', strtotime($person['end_date'])) : '';
                ...
                $sheet->setCellValue("M{$row}", $person['detailed_address'] ?? '');
            ...
            file_put_contents(base_path('error_log.txt'), 'VN Excel Error: ' . $e->getMessage() . "\n" . $e->getTraceAsString(), FILE_APPEND);
            Log::error('VN Import Excel generation error: ' . $e->getMessage() . ' trace: ' . $e->getTraceAsString());
            return null;
        }
    }
    ...
    private function generateForeignImportExcel($occupants, $submitter)
    {
        try {
            ...
            if (!file_exists($templatePath)) {
                file_put_contents(base_path('error_log.txt'), "Foreign Template not found: {$templatePath}\n", FILE_APPEND);
                Log::error("Template not found: {$templatePath}");
                return null;
            }
            ...
            // Save to storage
            $filename = 'danh_sach_mau_excel_completed_' . date('YmdHis') . '.xlsx';
            $tempPath = storage_path('app/dma/' . $filename);
            if (!file_exists(dirname($tempPath))) {
                mkdir(dirname($tempPath), 0755, true);
            }
            ...
            file_put_contents(base_path('error_log.txt'), 'Foreign Excel Error: ' . $e->getMessage() . "\n" . $e->getTraceAsString(), FILE_APPEND);
            Log::error('Foreign Import Excel generation error: ' . $e->getMessage() . ' trace: ' . $e->getTraceAsString());
            return null;
        }
    }
    ...
    private function buildSubmitterMessage($submitter, $item)
    {
        try {
           ...
           $this->sendMsg($mailRecord);
            ...
            $accommodationExcelPath = $this->generateAccommodationExcel($occupants, 'accommodation_register.xlsx');
            // Generate filled templates based on nationality
            $vnImportPath = $this->generateVnImportExcel($occupants, $submitter);
            $foreignImportPath = $this->generateForeignImportExcel($occupants, $submitter);
            ...
            if ($accommodationExcelPath && file_exists($accommodationExcelPath)) {
            ...
            return view('vgDormrequest-completed', [
                ...
                'link' => "http://gmo021.cansportsvg.com/ga/dma",
                'qrCodeUrl' => "http://gmo021.cansportsvg.com/api/vgDorm/getQRCode/dma/" . $item->id,
                'occupants' => $this->formatOccupantsData(json_decode($item->name ?? '[]', true), $item->reason)
            ])->render();
        }
    }
    ...
    private function getLocalizedNation($nationCode)
    {
        try {
            ...
            $nationItem = dormNationMdl_sub::where('code', $nationCode)->first();
            if (!$nationItem) return "Nation Code: {$nationCode}";
    ...
    public static function getFilteredRequests($userData, $includeDeputy = true)
    {
        ...
        if (!$approvalFlow) {
            $vfaController = new vfaCtrl();
            $request = new Request();
            $request->merge([
                'empno' => $userData['empno'],
                'app_code' => 'DMA',
                'location' => $userData['location'] ?? 'VN',
                'custom_data' => json_encode([])
            ]);
            $flowData = $vfaController->getFlowUpdated($request);
        ...
        foreach ($allRequests as $request) {
            try {
                if (!$request || !isset($request->status) || !isset($request->approval)) {
                    continue;
                }

                // Parse status & approval - handle both string and array inputs
                $status = is_array($request->status) ? $request->status : (is_string($request->status) ? json_decode($request->status, true) : null);

                $approval = is_array($request->approval) ? $request->approval : (is_string($request->approval) ? json_decode($request->approval, true) : null);
                ...
                if (!$status || !$approval || !is_array($submitter) || !is_array($nameArr)) {
                    continue;
                }

                $flowData = isset($request->flow_data) ? $request->flow_data : $approvalFlow;

                // Map nation cho occupants (chỉ lấy tiếng Anh)
                $occupants = [];
                if (is_array($nameArr)) {
                    foreach ($nameArr as $person) {
                        $nationId = isset($person['nation']) ? $person['nation'] : null;
                        $nationObj = $nationId && isset($nationMap[$nationId]) ? $nationMap[$nationId] : [];
                        $person['nation'] = isset($nationObj['en']) ? $nationObj['en'] : '';
                        $occupants[] = $person;
                    }
                }
                ...
                $targetLevel = null;
                if (is_array($status) && isset($status[0]['dept']) && $status[0]['dept'] === "false" && $status[0]['stt'] === "waiting dept") {
                    $targetLevel = 0;
                } else if (
                    is_array($status) && isset($status[0]['dept'], $status[1]['ga']) &&
                    $status[0]['dept'] === "true" && $status[0]['stt'] === "accept" &&
                    ($status[1]['ga'] === "false" || $status[1]['stt'] === "waiting ga")
                ) {
                    $targetLevel = 1;
                } else if (
                    is_array($status) && isset($status[1]['ga'], $status[2]['smp']) &&
                    $status[1]['ga'] === "true" && $status[1]['stt'] === "accept" &&
                    ($status[2]['smp'] === "false" || $status[2]['stt'] === "waiting smp")
                ) {
                    $targetLevel = 2;
                } else if (
                    is_array($status) && isset($status[2]['smp'], $status[3]['gm']) &&
                    $status[2]['smp'] === "true" && $status[2]['stt'] === "accept" &&
                    ($status[3]['gm'] === "false" || $status[3]['stt'] === "waiting gm")
                ) {
                    $targetLevel = 3;
                }
                ...
                if ($targetLevel !== null && isset($flowData[$targetLevel])) {
                    ...
                    if (isset($currentLevel['managers'])) {
                        foreach ($currentLevel['managers'] as $manager) {
                            if (strtoupper($manager['empno']) === strtoupper($userData['empno'])) {
                                $canApprove = true;
                                break;
                            }
                            if ($includeDeputy && isset($manager['deputies'])) {
                                foreach ($manager['deputies'] as $deputy) {
                                    if (strtoupper($deputy['empno']) === strtoupper($userData['empno'])) {
                                        $canApprove = true;
                                        break;
                                    }
                                }
                ...
                if ($canApprove) {
                    ...
                    $filteredRequest->approval = is_array($approval) ? $approval : [];
                    ...
                    $filteredRequest->email = isset($currentLevel['managers'][0]['email']) ? $currentLevel['managers'][0]['email'] : '';
                ...
                if (is_array($status) && is_array($approval)) {
                    foreach ($status as $idx => $level) {
                        if (isset($level['stt']) && strtolower($level['stt']) === 'deny') {
                ...
                if (is_array($approval)) {
                    foreach ($approval as $level) {
                        if (isset($level['empno']) && $level['empno'] === $userData['empno'] && strtolower($level['stt']) === 'accept') {
                            // Convert array to object for approved
                            $approvedRequest = new \stdClass();
                            $approvedRequest->id = $request->id;
                            $approvedRequest->key_in_date = $request->key_in_date;
    ...
    public static function getFilteredRequestsIfm($userData, $includeDeputy = true)
    {
        // 1. Lấy danh sách nation và build map id => nationObj
        $nationList = dormNationMdl::all();
        $nationMap = [];
        ...
        $userEmpno = $userData->empno ?? $userData['empno'] ?? null;
        if (!$userEmpno) {
            return [
                'title' => 'DMA',
                'color' => 'orange',
        ...
        // 2. Gom các flowKeys duy nhất để tối ưu số lần gọi API
        $flowKeys = [];
        foreach ($allRequests as $item) {
            $submitter = is_string($item->submitter) ? json_decode($item->submitter, true) : $item->submitter;
            $empno = $submitter['empno'] ?? null;
            if (empty($empno)) {
                continue;
            }
            $loc = strtolower($userData->location ?? 'vg');
            $localKey = 'dma|' . $loc . '|' . $empno;
            if (!isset($flowKeys[$localKey])) {
                $flowKeys[$localKey] = [
                    'empno' => $empno,
                    'location' => $loc
                ];
        ...
        // 3. Gọi API IFM để lấy dữ liệu luồng phê duyệt
        $promises = [];
        $client = new \GuzzleHttp\Client(['timeout' => 5]);
        $simDate = now()->format('Y-m-d');
        foreach ($flowKeys as $localKey => $info) {
            $promises[$localKey] = $client->createRequest('POST', 'http://gmo021.cansportsvg.com:10003/api/ifm-tracking/resolve', [
                'json' => [
                    'empno' => $info['empno'],
                    'app_code' => 'dma',
        ...
        $results = \GuzzleHttp\Pool::batch($client, $promises);

        $allFlowData = [];
        foreach ($promises as $localKey => $request) {
            $resObj = $results->getResult($request);
            if ($resObj && !($resObj instanceof \Exception)) {
                try {
                    $resp = $resObj;
                    $body = json_decode($resp->getBody()->getContents(), true);
                    $flow = data_get($body, 'result.flow_data') ?? data_get($body, 'flow_data');

                    $flowArr = $flow ? json_decode(json_encode($flow), true) : [];
                    $allFlowData[$localKey] = is_array($flowArr) ? $flowArr : [];
                } catch (\Throwable $e) {
                    $allFlowData[$localKey] = [];
                }
            } else {
                $allFlowData[$localKey] = [];
            }
        }
// 4. Gom các định danh của user
        $userIdentifiers = [];
        if (!empty($userData->empno)) {
            $userIdentifiers[] = strtolower($userData->empno);
        }
        if (!empty($userData['group_empno'])) {
            $userIdentifiers[] = strtolower($userData['group_empno']);
        }
        if (!empty($userData['syno_username'])) {
            $userIdentifiers[] = strtolower($userData['syno_username']);
        }
        $userIdentifiers = array_unique($userIdentifiers);
        ...
        foreach ($allRequests as $request) {
            try {
                ...
                $submitterEmpno = $submitter['empno'] ?? null;
                if (!$submitterEmpno) {
                    continue;
                }

                $loc = strtolower($userData->location ?? 'vg');
                $localKey = 'dma|' . $loc . '|' . $submitterEmpno;
                $flowData = $allFlowData[$localKey] ?? [];

                if (empty($flowData)) {
                    continue;
                }

                // Map nation cho occupants
                $occupants = [];
                if (is_array($nameArr)) {
                    foreach ($nameArr as $person) {
                        $nationId = isset($person['nation']) ? $person['nation'] : null;
                ...
                // Kiểm tra quyền duyệt bằng flowData của IFM
                $canApprove = false;
                $email = '';
                if ($targetLevel !== null && isset($flowData[$targetLevel])) {
                    $currentLevel = $flowData[$targetLevel];
                    if (isset($currentLevel['managers'])) {
                        foreach ($currentLevel['managers'] as $manager) {
                            if (isset($manager['empno']) && in_array(strtolower($manager['empno']), $userIdentifiers)) {
                                $canApprove = true;
                                $email = $manager['email'] ?? '';
                                break;
                            }
                            if ($includeDeputy && isset($manager['deputies']) && is_array($manager['deputies'])) {
                                foreach ($manager['deputies'] as $deputy) {
                                    if (
                                        isset($deputy['empno']) &&
                                        in_array(strtolower($deputy['empno']), $userIdentifiers) &&
                                        ($deputy['status'] ?? false) === true
                                    ) {
                                        $canApprove = true;
                                        $email = $deputy['email'] ?? '';
                                        break;
    ...
    public function getProcessedRequestsByEmpno(Request $req)
    {
        try {
            ...
            foreach ($allRequests as $request) {
                ...
                $request->submitter = is_string($request->submitter) ? json_decode($request->submitter, true) : $request->submitter;
                $nameArr = is_string($request->name) ? json_decode($request->name, true) : $request->name;
                ...
                foreach ($status as $index => $level) {
                    if (isset($level['stt']) && strtolower($level['stt']) === 'deny') {
                        if (isset($approval[$index]['empno']) && $approval[$index]['empno'] === $req->empno) {
                ...
                if (!empty($approval)) {
                    foreach ($approval as $level) {
                        if (isset($level['empno']) && $level['empno'] === $req->empno) {
