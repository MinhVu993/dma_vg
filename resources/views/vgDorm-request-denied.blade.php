...
<body>
    <div class="header">
        ...
        <p class="status-denied">Đơn đăng ký của bạn đã bị từ chối/您的申請已被拒絕</p>
        <p>Từ chối bởi/拒絕者: {{ $denier_level }}</p>
        <p>Bộ phận/部门: {{ $department }}</p>
    </div>

    <div class="deny-reason">
        <h3>Lý do từ chối/拒絕原因:</h3>
        <p>{{ $deny_reason }}</p>
    </div>

    <h3>Chi tiết đơn/申請詳情:</h3>
    ...
    <table>
        ...
        <tr>
            <td>Gender / 性別</td>
            @foreach($occupants as $occupant)
                <td>{{ $occupant['gender'] === 'M' ? 'Nam/男' : 'Nữ/女' }}</td>
            @endforeach
        </tr>
        <tr>
    ...
    <p>Kiểm tra trạng thái đơn tại/在此查看申請狀態:</p>
