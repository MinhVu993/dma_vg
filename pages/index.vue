<template>
        ...
        <v-col cols="12">
            <v-card>
                ...
                <v-toolbar dense color="teal darken-2" dark>
                    ...
                    <v-btn @click="openNewRequestDialog" :title="$t('New request')" color="warning" small light nuxt>
            ...
            <v-tabs background-color="teal darken-2" color="yellow" dark v-model="statusFilter">
                <v-tab> {{ $t("All") }} </v-tab>
                <v-tab v-show="hasTodoItems">
                    <v-badge
                        :value="todoCount > 0"
                        color="red"
                        :content="todoCount > 9 ? '9+' : todoCount"
                    >
                        {{ $t("To do") }}
                    </v-badge>
                </v-tab>
                <v-tab>
                    <v-badge
                        ...
                        :content="processingCount > 9 ? '9+' : processingCount"
                    >
                        {{ $t("Processing") }}
                    </v-badge>
                </v-tab>
            ...
            :footer-props="{ showFirstLastPage: true, firstIcon: 'mdi-arrow-collapse-left', lastIcon: 'mdi-arrow-collapse-right', prevIcon: 'mdi-minus', nextIcon: 'mdi-plus', 'items-per-page-text': $t('Rows per page') }">
    ...
    <template v-slot:item.status="{ item }">
        <v-tooltip bottom v-if="!isCompleted(item) && !hasAnyDeniedStatus(item)" color="teal darken-1">
            <template v-slot:activator="{ on, attrs }">
...
<template v-slot:item.room_no="{ item }">
    <div class="d-flex align-center justify-center">
        <v-tooltip bottom v-if="getRoomCount(item) > 0" color="blue-grey darken-3">
            <template v-slot:activator="{ on, attrs }">
                <v-chip v-bind="attrs" v-on="on" small color="blue-grey lighten-5" text-color="blue-grey darken-3" class="font-weight-bold" label>
                    <v-icon left small color="blue-grey darken-2">mdi-home-circle-outline</v-icon>
                    {{ getDisplayedRooms(item) }}
                </v-chip>
            </template>
            <span class="text-caption">{{ getFullRoomsList(item) }}</span>
        </v-tooltip>
    ...
    <v-card>
        <v-form v-model="formValid" ref="appForm">
            <v-toolbar dark color="teal darken-2">
                ...
                <v-spacer></v-spacer>
                <v-btn color="primary" @click="saveSelectedApps" class="mr-2" :disabled="isViewDetail">
            ...
            <v-container fluid class="pa-1">
                ...
                :status="selectedItem ? selectedItem.status : null"
                @sendAppFlow="handleAppFlow" style="font-size: 10px;"></VfaAppFlow>
                <v-card flat>
                    ...
                    <v-card-text>
                        <v-row>
                            <v-col cols="12" sm="3">
                                ...
                                :label="$t('Key in')" readonly outlined dense disabled hide-details
                                prepend-inner-icon="mdi-calendar"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="3">
                                <v-text-field :value="selectedItem ? submitterInfo.name : activeUser.name" hide-details
                                :label="$t('Created Name')" outlined dense disabled></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="3">
                                <v-text-field :value="selectedItem ? submitterInfo.dept : activeUser.dept" hide-details
                                :label="$t('Dept')" outlined dense disabled></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="3">
                                <v-text-field :value="selectedItem ? submitterInfo.ext : activeUser.ext" hide-details
...
<v-col cols="12" sm="6" :md="(!contact.location || contact.location.loc !== 'VG') ? 3 : 4">
    <v-textarea v-model="contact.notes" :label="$t('Notes')" outlined auto-grow clearable
                ...
                <template v-slot:default>
                    ...
                    <tbody>
                        <tr v-for="(approval, index) in selectedApprovals" :key="index">
                            ...
                            <td>
                                <v-chip v-if="approval.stt" small label :color="getApprovalStatusColor(approval.stt)">
                                    <v-icon left small>
                                        {{ approval.stt === 'accept' ? 'mdi-check-circle' : approval.stt === 'deny' ? 'mdi-close-circle' : 'mdi-help-circle' }}
                                    </v-icon>
                                    {{ approval.stt }}
                                </v-chip>
                            </td>
...
<v-dialog v-model="denyDialog" max-width="500">
    <v-card>
        ...
        <v-card-text class="pt-4">
            <v-textarea v-model="denyReason" :label="$t('Reason for denial')" auto-grow
...
import VfaAppFlow from '@/components/AppFlow.vue';
...
export default {
    ...
    data() {
        return {
            ...
            { text: this.$t('Submitter'), value: "submitter",   },
            { text: this.$t('Register date'), value: "key_in_date",  align: 'center' },
            { text: this.$t('Room Occupancy'), value: "name",  align: 'center' },
            { text: this.$t('Room No'), value: "room_no",  align: 'center' },
            { text: this.$t('Reason'), value: 'reason', align: 'center', },
            // { text: this.$t('QR'), value: "qr_code",  align: 'center' },
            { text: this.$t('Status'), value: "status",  align: 'center' },
            { text: this.$t('Approval status'), value: "approval",  align: 'center' },
            ],
            dromData: [],
            activeUser: {
                name: '',
    ...
    computed: {
        baseVisibleDormData() {
            if (!this.dromData || !this.dromData.length) return [];
            return this.dromData.filter(item => {
                if (this.activeUser?.role === 'admin') return true;
                
                try {
                    const submitter = JSON.parse(item.submitter);
                    // Check if creator or same dept/hight_dept
                    if (submitter.empno === this.activeUser?.empno || 
                        submitter.dept === this.activeUser?.dept ||
                        (submitter.hight_dept && submitter.hight_dept === this.activeUser?.hight_dept)) {
                        return true;
                    }
                    
                    // Check if user is an approver for this item
                    const itemAppFlow = this.getAppFlowForItem(item);
                    if (itemAppFlow && itemAppFlow.length) {
                        const isApprover = itemAppFlow.some(level => 
                            level.managers.some(m => 
                                this.isEmpnoMatch(m.empno) || 
                                m.email === this.activeUser?.email ||
                                m.deputies?.some(d => 
                                    (this.isEmpnoMatch(d.empno) || d.email === this.activeUser?.email) && 
                                    (d.status === true || d.status === undefined)
                                )
                            )
                        );
        ...
        todoCount() {
            if (!this.baseVisibleDormData || !this.baseVisibleDormData.length) return 0;
            const currentUserEmpno = this.activeUser?.empno;
            const currentUserEmail = this.activeUser?.email;
            if (!currentUserEmpno) return 0;

            let count = 0;
            this.baseVisibleDormData.forEach(item => {
                try {
                    const status = JSON.parse(item.status);
                    const submitterDept = JSON.parse(item.submitter).dept;
                    
                    const itemAppFlow = this.getAppFlowForItem(item);
                    if (!itemAppFlow || !itemAppFlow.length) return;
                    
                    const userLevels = {
                        isLevel1: itemAppFlow[0]?.managers.some(m => {
                            const hasDeptsAccess = Array.isArray(m.dept_code)
                                ? m.dept_code.some(code => code === submitterDept || submitterDept.startsWith(code))
                    ...
                    const isTodo = (currentLevel === 1 && userLevels.isLevel1) ||
        ...
        filteredDormData() {
            ...
            // Apply base visibility permissions
            let baseData = this.baseVisibleDormData;

            if (requestId) {
                return baseData.filter(item => item.id === requestId);
            }
            
            let filtered = this.statusFilter === 0 ? [...baseData] :
            baseData.filter(item => {
                try {
                    const status = JSON.parse(item.status);
                    const currentUserEmpno = this.activeUser.empno;
                    const currentUserGroup = this.activeUser.group;
                    ...
                    if (this.statusFilter === 1) {
                        ...
                        // Use per-submitter AppFlow
                        const itemAppFlow = this.getAppFlowForItem(item);
                        if (!itemAppFlow || !itemAppFlow.length) return false;
            ...
            // Auto-switch to ALL tab when TO DO has no items
            if (this.statusFilter === 1 && filtered.length === 0 && this.dromData.length > 0) {
                this.$nextTick(() => {
                    this.statusFilter = 0;
                });
            }
    ...
    methods: {
        ...
        async getNation() {
            try {
                ...
                this.nationalityList = res.data.dormNationSub
            } catch (error) {
                console.log(error);
            }
        },
        ...
        handleAppFlow(appFlow) {
            ...
            this.dataAppFlow = appFlow.map((item, index) => ({
                ...
                managers: item.managers.map((x) => {
                    let deptCode = [];
                    if (Array.isArray(x.dept_names)) {
                        deptCode = x.dept_names;
                    } else if (x.dept_code) {
                        deptCode = [x.dept_code];
                    } else if (x.division_code) {
                        deptCode = [x.division_code];
                    }
                    return { email: x.email, empno: x.empno, name: x.name, dept_code: deptCode, deputies: x.deputies };
                }),
            }));
            ...
            try {
                ...
                const data = {
                    ...
                    submitter: JSON.stringify({
                        ...
                        hight_dept: this.activeUser.hight_dept,
        ...
        async getDormData() {
            try {
                ...
                // Pre-fetch AppFlow for all unique submitters
                await this.prefetchAppFlows();
        ...
        async prefetchAppFlows() {
            const uniqueSubmitters = new Set();
            for (const item of this.dromData) {
                try {
                    const submitter = JSON.parse(item.submitter);
                    if (submitter.empno) {
                        uniqueSubmitters.add(submitter.empno);
                    }
                } catch (e) { /* skip */ }
            }
            
            const promises = [];
            for (const empno of uniqueSubmitters) {
                if (!this.appFlowCache[empno]) {
                    promises.push(this.fetchAppFlowForSubmitter(empno));
                }
            }
            ...
            try {
                ...
                if (res.data) {
                    if (res.data.result && res.data.result.flow_data) {
                        flowData = res.data.result.flow_data;
                    } else if (res.data.flow_data) {
                        flowData = res.data.flow_data;
                    } else if (Array.isArray(res.data.result)) {
                        flowData = res.data.result;
                    } else if (Array.isArray(res.data)) {
                        flowData = res.data;
                    }
                }
                if (flowData) {
                    const parsed = flowData.map((item, index) => ({
                        level: 'level' + (index + 1),
                        managers: item.managers.map((x) => {
                            let deptCode = [];
                            if (Array.isArray(x.dept_names)) {
                    ...
                    this.$set(this.appFlowCache, empno, parsed);
                }
            } catch (e) {
                console.error('Failed to fetch AppFlow for', empno, e);
            }
        },
        getAppFlowForItem(item) {
            try {
                const submitter = JSON.parse(item.submitter);
                return this.appFlowCache[submitter.empno] || this.dataAppFlow;
            } catch (e) {
                return this.dataAppFlow;
            }
        },
        /**
         * Check if a given empno matches the current user
         * Compares against both empno (login) and group (group_empno from approval system)
         */
        ...
        async confirmDeny() {
            ...
            try {
                ...
                const itemAppFlow = this.getAppFlowForItem(item);
                const hasPermission = itemAppFlow[levelIndex]?.managers.some(m =>
                this.isEmpnoMatch(m.empno) || m.email === this.activeUser?.email ||
                m.deputies?.some(d => (this.isEmpnoMatch(d.empno) || d.email === this.activeUser?.email) &&
                (d.status === true || d.status === undefined))
                );
                ...
                const requestData = {
                    ...
                    dataAppFlow: itemAppFlow.map(level => ({
                        level: level.level,
        ...
        canTakeAction(item) {
            try {
                ...
                if (!currentUserEmail && !currentUserEmpno) {
                    return false;
                }
                ...
                // Check if user is a manager or deputy at any level (by empno, group, OR email)
                const isManagerOrDeputy = (m) =>
                    this.isEmpnoMatch(m.empno) ||
                    m.email === currentUserEmail ||
                    m.deputies?.some(d =>
                        (this.isEmpnoMatch(d.empno) || d.email === currentUserEmail) &&
                        (d.status === true || d.status === undefined)
                    );
                
                const level1Permission = itemAppFlow[0]?.managers.some(isManagerOrDeputy);
                const level2Permission = itemAppFlow[1]?.managers.some(isManagerOrDeputy);
                const level3Permission = itemAppFlow[2]?.managers.some(isManagerOrDeputy);
                const level4Permission = itemAppFlow[3]?.managers.some(isManagerOrDeputy);
        ...
        canAccept(item) {
            try {
                ...
                // Use per-submitter AppFlow
                const itemAppFlow = this.getAppFlowForItem(item);
                if (!itemAppFlow || !Array.isArray(itemAppFlow)) return false;
                
                // Update level 1 check to include department validation
                const isLevel1 = itemAppFlow[0]?.managers?.some(m => {
                    const hasDeptsAccess = Array.isArray(m.dept_code)
                        ? m.dept_code.some(code => code === submitterDept || submitterDept.startsWith(code))
                        : (m.dept_code?.includes(submitterDept) || false);
                    const isManagerOrDeputy =
                ...
                const isLevel2 = itemAppFlow[1]?.managers?.some(m =>
                this.isEmpnoMatch(m.empno) ||
                m.deputies?.some(d => this.isEmpnoMatch(d.empno) && (d.status === true || d.status === undefined))
                );
                const isLevel3 = itemAppFlow[2]?.managers?.some(m =>
                this.isEmpnoMatch(m.empno) || m.email === currentUserEmail ||
                m.deputies?.some(d => (this.isEmpnoMatch(d.empno) || d.email === currentUserEmail) && (d.status === true || d.status === undefined))
                );
                const isLevel4 = itemAppFlow[3]?.managers?.some(m =>
                this.isEmpnoMatch(m.empno) || m.email === currentUserEmail ||
                m.deputies?.some(d => (this.isEmpnoMatch(d.empno) || d.email === currentUserEmail) && (d.status === true || d.status === undefined))
                );
        ...
        hasAnyDeniedStatus(item) {
            try {
                const status = JSON.parse(item.status);
                return status.some(s => s.stt === "deny");
            } catch {
                return false;
            }
        },
        ...
        getLocalizedNation(nationId) {
            try {
                if (!nationId) return '';
                const nationItem = this.nationalityList.find(item => String(item.code) === String(nationId));
        ...
        getManagerInfo(item) {
            try {
                ...
                if (status.some(s => s.stt === "deny")) return '';
                ...
                const itemAppFlow = this.getAppFlowForItem(item);
                const levelData = itemAppFlow?.find(l => l.level === level);
                ...
                levelData.managers.forEach(manager => {
                    ...
                    if (manager.deputies && manager.deputies.length > 0) {
                        const activeDeputies = manager.deputies.filter(d => d.status === true || d.status === undefined);
        ...
        handleNationalityChange(value, index) {
            if (this.contacts[index]) {
                this.contacts[index].nationality = value;
                this.contacts[index].citizenId = '';
                this.contacts[index].passportNumber = '';
            }
        },
